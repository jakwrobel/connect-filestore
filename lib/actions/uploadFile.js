...
async function doStreamRequests({
  ...
  for (let i = 0; i < numberOfChunks; i++) {
    ...
    externalThis.logger.info('function chunkStart', chunkStart)
    externalThis.logger.info('function chunkEnd', chunkEnd)
    externalThis.logger.info('function fileSize', fileSize)
    externalThis.logger.info('function chunk', chunk)
    externalThis.logger.info('function chunk length', chunk.length)
    externalThis.logger.info('function chunk string', chunk.toString("base64"))
    ...
    const config = {
      ...
      headers: {
        ...
        "content-length": `${chunkString.length}`
      },
      body: `${chunkString}`,
    }
