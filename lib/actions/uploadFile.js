...
async function doStreamRequests({
  ...
  for (let i = 0; i < numberOfChunks; i++) {
    ...
    logger.info('chunkStart', chunkStart)
    logger.info('chunkEnd', chunkEnd)
    logger.info('chunk', chunk)
    logger.info('fileSize', fileSize)
    logger.info('chunk-length', chunk.length)
    logger.info('requestStartByte',requestStartByte)
    logger.info("content-range",`bytes ${requestStartByte + i*chunkSize}-${requestStartByte + (i+1)*chunkEnd - 1}/${fileSize}`)
    await axios.patch(`${resourceServerUrl}/api/v2/file/${fileId}`, chunk, {
      headers: {
        "x-api-key": apiKey,
        "x-dxp-tenant": tenantId,
        "content-type": "application/octet-stream",
        "content-range": `bytes ${requestStartByte + i*chunkSize}-${requestStartByte + (i+1)*chunkEnd - 1}/${totalSize}`,
        "content-length": `${chunk.length}`,
      },
    });
  }
...
try {
  ...
  attachmentSize = msg?.attachments["image.jpg"]?.size
...
this.logger.info('imageData', dataStream.data._readableState.buffer[0])
...
this.logger.info('readableState.buffer length',dataStream.data._readableState.buffer.length)
    ...
    for (let i = 0; i < dataStream.data._readableState.buffer.length; i++) {
      const requestStartByte = i===0 ? 0 : dataStream.data._readableState.buffer[i].length + 1
