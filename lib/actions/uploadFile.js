...
async function doStreamRequests({
  ...
  for (let i = 0; i < numberOfChunks; i++) {
    logger.info('executed', i, numberOfChunks)
    logger.info('fikeSize', fileSize)
...
this.logger.info('dataStream.data._readableState.buffer',dataStream.data._readableState.buffer)
    ...
    await doStreamRequests({
      ...
      tenantId,
      fileId,
      fileBuffer: dataStream.data._readableState.buffer[2],
      fileSize: dataStream.data._readableState.buffer[2].length,
      logger: this.logger
    });
