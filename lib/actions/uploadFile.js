...
async function doStreamRequests({
  ...
  for (let i = 0; i < numberOfChunks; i++) {
    ...
    externalThis.logger.info('this is the chunk string', chunkString)
    externalThis.logger.info('content-range', `bytes ${chunkStart}-${chunkEnd - 1}/${fileSize}`)
    externalThis.logger.info('content-length', `${chunk.length}`)
    ...
    await fetch(`${resourceServerUrl}/api/v2/file/${fileId}`,{
      method: "PATCH",
      headers: {
        "x-api-key": apiKey,
        "x-dxp-tenant": tenantId,
