...
try {
  dataStream = await attachmentProcessor.getAttachment(msg.attachments["_2LH8086.jpg"].url, 'stream')
  this.logger.info('type of the dataStream', typeof dataStream)
  this.logger.info('dataStream.data keys', Object.keys(dataStream.data))
  this.logger.info('dataStream', Object.prototype.toString.call(dataStream.data))
  this.logger.info('dataStream strinigified', JSON.stringify(dataStream.data));
  this.logger.info('dataStream _readableState', typeof dataStream.data._readableState, JSON.stringify(dataStream.data._readableState));
  this.logger.info('File received, trying to parse CSV')
} catch (err) {
  this.logger.error(`URL - "${msg.attachments["_2LH8086.jpg"].url}" unreachable: ${err}`);
  this.emit('error', `URL - "${msg.attachments["_2LH8086.jpg"].url}" unreachable: ${err}`)
  this.emit('end')
  return
}
if (dataStream instanceof ReadableStream) {
  this.logger.info('This is ReadableStream');
} else if (dataStream instanceof WritableStream) {
  this.logger.info('This is WritableStream');
} 
else if (dataStream instanceof TransformStream){
