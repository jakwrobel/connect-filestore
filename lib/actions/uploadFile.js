...
try {
  dataStream = await attachmentProcessor.getAttachment(msg.attachments["file.svg"].url, 'stream')
  this.logger.info('dataStream _readableState',  dataStream.data._readableState.buffer[0])
} catch (err) {
  this.logger.error(`URL - "${msg.attachments["file.svg"].url}" unreachable: ${err}`);
  this.emit('error', `URL - "${msg.attachments["file.svg"].url}" unreachable: ${err}`)
  this.emit('end')
  return
}
