...
this.logger.info(JSON.stringify(msg))
  ...
  try {
    dataStream = await attachmentProcessor.getAttachment(body.attachments["_2LH8086.jpg"].url, 'stream')
    this.logger.info('File received, trying to parse CSV')
  } catch (err) {
    this.logger.error(`URL - "${body.attachments["_2LH8086.jpg"].url}" unreachable: ${err}`);
    this.emit('error', `URL - "${body.attachments["_2LH8086.jpg"].url}" unreachable: ${err}`)
    this.emit('end')
    return
  }
  this.logger.info(JSON.stringify(dataStream))
